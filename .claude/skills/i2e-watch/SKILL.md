...
Watch mode does not change what `status:` means — `active` already means
"in the IDEA loop, will be developed" (spec §2.1). It only removes the
delay. Three points the operator must hold:

- **`draft → active` and a `version` bump both mean "build now".** Keep a
  capability `draft` until it is ready to be worked on. `draft` intents are
  skipped entirely — editing one never triggers a dispatch — so a draft is
  a free scratchpad.
- **"Ready but blocked" is `depends_on:`, not `draft`.** A capability that
  is ready in principle but waiting on another should be `active` with
  `depends_on:` set. The watcher respects the graph and holds the child
  back until its parent is developed.
- **Bump `version` last.** On an already-active intent the *save* is the
  trigger. Bump `version` (and `updated`) only once the edit is complete —
  preflight rejects a malformed intent, but a half-finished *valid* one
  would still build.
