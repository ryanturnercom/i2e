...
short-circuits if there are no active capabilities AND no resolved
pendings on disk — `decide()` returns `Shippable()` immediately, skipping
the pending walk, current.yaml reads, version-bump comparisons, and
target-window math. The preflight cache (`.i2e/.preflight_cache.json`) is
...
The sections above describe the system as it ships today. Two draft
intents remain unimplemented. Each links to its source file under
`.i2e/intents/` and will be folded into the numbered sections above
when it reaches `shippable`.

### 12.1 `shipped` status for completed capabilities
...
do **not** auto-demote — see 12.2. Status writes happen through the same
narrow orchestrator carve-out used by `runtime:` in §6.3. The report
renders shipped capabilities in their own "Shipped (N)" section. Lands
changes in §2.1, §6.1, §8.

### 12.2 `i2e-regression` skill — periodic case re-validation
Intent: [`i2e-regression`](../.i2e/intents/i2e-regression.md)

Companion to 12.1. The IDEA loop has no mechanism to re-run pytest cases
...
`/schedule`, or wire to CI. Depends on 12.1. Lands changes in §4.1, §9,
and Appendix B.

### 12.3 Activation order

12.1 ships first; 12.2 must come after it (it uses the `shipped →
active` demotion path).

### 12.4 Recently activated (history)
