#!/usr/bin/env bash
# i2e-serve — stop any running server, then start a fresh one (foreground).
#
...
# Best-effort stop; no-op if nothing is running.
"$PYTHON" -m i2e_core.serve stop --root "$ROOT" || true
